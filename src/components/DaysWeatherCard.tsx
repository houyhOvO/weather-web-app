...
export function DaysWeatherCard({ data }: DaysWeatherProps) {
  ...
  return (
    <div className="flex flex-col items-center p-6 min-w-32.5 bg-white rounded-3xl shadow-lg border border-slate-100 transition-all duration-300 hover:shadow-xl hover:-translate-y-1">
      <div className="text-center mb-3">
        <p className="text-[10px] uppercase tracking-widest text-slate-500 font-bold">
          {dayName}
        </p>
        <p className="text-[10px] text-slate-500 font-bold">{dateNum}</p>
      </div>

      <div className="my-4">
        <WeatherIcon name={data.icon} className="w-10 h-10" />
      </div>
