...
export function WeatherIcon({ name, className, ...props }: WeatherIconProps) {
  const config = WEATHER_ICONS[name as keyof typeof WEATHER_ICONS] || {
  ...
  return (
    <IconComponent className={`${config.color} ${className}`} {...props} />
  );
}
